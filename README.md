...
TwitterVisualiser is essentially an application which allows its user to view *crime* data pulled from Twitter.
This data is collected is filtered via a *Natural Language Processing (NLP)* system, which for *Data Visualisation*,
...
- Trajectory model based of accurate data sets.
...
> Feel free to install `yarn` if you not wish to use `npm@4`.
...
In **Windows** things are a little different. You will not be able to test it an *iOS* emulator unless you have an Apple device.
Which really sucks, but that's how it is! To run the application here, you need to have an *Android Virtual Device* setup. Which
is pretty much *Android Studio*. If you don't want to do that, you can plug in an *android* device in debugging mode and run
...
you would need to press the key `"a"` to restart the deployment, which allows the edits to show in your application. Meaning, hot-reload,
does not exist in Windows. Sad reacts. ~ At least it works, and it's cross-platform!
