...
const connectFailure = function() {
    console.log("This will abort the NodeJS process.");
    process.exit(1);
}

init();

//Create all the connections to the DB and export them for other modules
function init() {
...
//Used for getting data from the DB
...
//Get all tweets from specified DB, useful for taking a backup in JSON without having to copy the actual DB folder
exports.getAll = async (req, res) => {
    let result = {};

    await this.getStoredTweets(req.params.location, {}, 0, 0).then((res) => {
