...
export function pointToDensityGridData({
  ...
  const opts = _getGPUAggregationParams({
    gridData,
    cellSize,
    worldOrigin
  });
...
function _getGPUAggregationParams({
  gridData,
  cellSize,
  worldOrigin
}) {
  ...
  const originY = _alignToCell(yMin - worldOrigin[1], cellSize[1]) + worldOrigin[1]; // Setup transformation matrix so that every point is in +ve range
