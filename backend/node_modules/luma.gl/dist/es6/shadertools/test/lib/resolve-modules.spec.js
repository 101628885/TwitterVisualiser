...
const getDependencyGraph = TEST_EXPORTS.getDependencyGraph;
const fp32 = {
  name: 'fp32-test'
};
const fp64 = {
...
test('ShaderModules#getShaderDependencies', t => {
  ...
  t.deepEqual(result.map(module => module.name), [fp32.name, project.name, fp64.name, project64.name], 'Module order is correct');
  t.end();
});
